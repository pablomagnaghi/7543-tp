...
# $1 -> ip del servidor
openvpn --config ./cliente.conf --remote $1 --port 1308 --ifconfig $IP_DNS_ROOT $MASC_A &
...
echo "domain mercedes.baires.dc.fi.uba.ar" > /etc/resolv.conf
echo "search mercedes.baires.dc.fi.uba.ar" >> /etc/resolv.conf
